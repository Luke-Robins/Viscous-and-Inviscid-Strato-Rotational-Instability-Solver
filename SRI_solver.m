function [ E_, u_k, v_k, w_k, rho_k, P_k ] = SRI_solver( eta_, mu_, m_, k_, Re_, N_, T_ )
%SRI_solver
%Dr Luke Robins 2019 luke.robins@cantab.net
%
%   This MatLab program numerically evaluates the Strato-Rotational
%Instability (SRI) system for any given set of parameters.
%
...
%
%  - Re_
%    This is the Reynolds number of the Taylor-Couette system, defined as
%    the gap-width multiplied by the rotation speed of the inner cylinder,
...
%       
%       If T_ is unsupplied, or if T_=-1 is given, then SRI_solver.m will
%       instead estimate the appropriate value of T_ to be used. This
%       estimate is based on the magnitude of Re_, since larger values of
%       Re_ typically require higher resolutions.
%
%
%Output Parameters:
%  - E_
...
if (nargin<7)||(T_==-1)
    ...
    %   Re_ typically require higher resolutions.
    %
    %   The estimate is based on the observation that smaller values of Re_
    %   can often be solved with shorter Chebyshev expansions, which are
    %   quicker. This approach can therefore significantly cut down the
    %   run-time of routines that call SRI_solver.m many times, such as
    %   SRI_Find_Re, SRI_Find_k and SRI_Find_mk.
    %
    %   However direct control over T_ may be desired instead, in which
    %   case it can be directly specified.
    
    T_=1+floor(40*(Re_^0.2));
    
    %   This formula was derived based on many evaluations of SRI_solver.m
    %   across the [eta_,mu_,m_,k_] parameter space for 0<eta_<1, 0<mu_<1,
    %   m_=O(1) and k_=O(10). That said, it is obviously impossible to
    %   exhaustively test all regions of an infinite and continuous
    %   parameter space, so it is possible that the estimation may prove to
    %   be unreliable in some cases.
end


    function [LHS_box, RHS_box, Info_] = eqns_( Eqn_e, EigFun_e, T_e )
        %The MatLab function defined in SRI_equations.m needs to be redefined to
        %fit into the format required by the GenEig.m method.
        [LHS_box, RHS_box, Info_] = SRI_equations( Eqn_e, EigFun_e, T_e, mu_, eta_, m_, k_, Re_, N_ );
    end

%Use the GenEig.m to construct the appropriate matrices:
...
%Filter out any results spurious Eigen-Function expansions:
...
if not(isempty(Safe_Es_))
    %Find the result with the largest real growth rate:
    [ E_, EigVect_ ] = Largest_E( Safe_Es_, Safe_Evs_ );
    
    %Extract the individual Eigen-Functions for that eigenfunction:
...
fprintf('\n E_= %6.4g;\t eta_= %5.4g;\t mu_= %5.4g;\t m_= %3d;\t k_= %6.4g;\t Re_= %6.0f;\t N_= %4.2g;\t T_= %7g; \n ', real(E_), eta_, mu_, m_, k_, Re_, N_, T_ );
